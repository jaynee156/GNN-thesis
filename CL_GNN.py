    ...
    def insert_iter(self, p, node_id):
        ...
        while True:
            ...
            if d_p_Q == 0.0 :    # already there, no need to insert
                ...
                return
            elif d_p_Q > self.base**i or i==-5: # the found parent should be right
                # print(d_p_Q, "i is :", i)
                # or i==-6 )and used == 1)
                break
...
def loss_function(q, k, que, gpu):
    ...
    for i in range(len(run)):
        sum = torch.zeros(1).to(device=gpu)
        ...
        for j,x in enumerate(que[i]):
            ...
            mul = torch.div(torch.mm(a, z), τ)
            mul = torch.exp(mul)
            # print("\n neg ", mul)
            sum =  sum.add(mul)

        denominator = pos.add(sum)
        loss = -torch.log(torch.div(pos, denominator))
